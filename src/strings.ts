...
export const kebab = (source: string): string => source
    .replace(/([a-z])([A-Z])/g, '$1-$2')
    .replace(/[\s_]+/g, '-')
    .toLowerCase()
...
export const mask = (source: string, mask: string = '*', show: number = 0, end: number = 0): string => {
    if (show + end >= source.length) {
        return source;
    }
    const maskedLength = source.length - show - end;
    const maskedPart = mask.repeat(maskedLength);
    return maskedPart + source.slice(maskedLength, source.length - end) + source.slice(source.length - end);
}
...
export const replaceLast = (source: string, target: string, replacement: string): string => {
...
export const singular = (source: string): string => {
    ...
    return rules.reduce((result, [regex, replacement]) => 
        result.replace(regex, replacement), source);
}
...
export const snake = (source: string): string => source
    .replace(/\s+/g, '_')
    .replace(/([a-z])([A-Z])/g, '$1_$2')
    .replace(/[-_]+/g, '_')
    .toLowerCase()

export const squish = (source: string): string => source.trim().replace(/\s+/g, ' ')
...
export const substr = (source: string, start: number, length: number = 0): string => length === 0 ? source.substring(start) : source.substring(start, start + length)
...
export const words = (source: string, words: number, replacement: string = '...'): string => {
    const wordArray = source.split(/\s+/);
    return (wordArray.length <= words) ? source : wordArray.slice(0, words).join(' ') + replacement;
}